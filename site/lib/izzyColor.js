...
function iColorShow(i, i2) {
	...
	jQuery("#izzyColor").css({
		'top' : eIC.top + (jQuery("#" + i).outerHeight()) + "px",
		'left' : (eIC.left) + "px",
		'position' : 'absolute'
	}).fadeIn("fast");
	jQuery("#izzyColorBg").css({
		'opacity' : '0.6',
	...
	jQuery(".izzyColors a, #colorPreview").unbind().mouseover(function() {
		...
		var izzyColor = "#" + jQuery(this).attr("rel");
		jQuery("#" + i).val(izzyColor).css({
			'position' : 'static',
			"background" : izzyColor
		});
...
this.izzyColor = function() {
	...
	for (i = 0; i < temp.length; i++) {
		output += '<a href="#" rel="' + temp[i] + '" style="height:10px;width:10px;display:block;text-indent:-9999px;float:left;background:#' + temp[i]
				+ ';margin:0 1px 1px 0"></a>';
	}
	output += '</div>';
	...
	jQuery("input.izzyColor").each(
			function(i) {

				if (i == 0) {

					jQuery(document.createElement("div")).attr("id", "izzyColor").css('display', 'none').html(output).appendTo("body");
					jQuery(document.createElement("div")).attr("id", "izzyColorBg").click(function() {
						jQuery("#izzyColorBg").hide();
						jQuery("#izzyColor").fadeOut();
						jQuery("input.izzyColor").css({
							'position' : 'static'
						});
					}).appendTo("body");
				...
				if (jQuery(this).attr("class").indexOf("izzyColorAdded") < 0) {
					jQuery(this).addClass("izzyColorAdded");
					jQuery(this).css("backgroundColor", jQuery(this).val()).after(
							'<a href="javascript:void(null)" id="icp_' + this.id + '" onclick="iColorShow(\'' + this.id + '\',\'icp_' + this.id + '\')"><img src="' + imageUrl
