...
/**
 * 
 * @Description The top-level aurora namespace. All public methods and fields
 *              should be registered on this namespace object.
...
/**
 * 
 * Returns a prototype object suitable for extending the given class <tt>f</tt>.
...
/**
 * @class The basefunction for the graph initilization. Initializes the panel
 *        and sets parentDimensions and property values
 * @param {json}
 *            graphDef is the current graph Dimension This method should be the
 *            first method that should be called when creating any kind of
 *            graph. Creates the panel, sets its dimensions, labels and property
 *            values and returns.
 */
AR.Graph = function() {
	var self = this;
...
AR.Graph.prototype.initialize = function(graphDef) {
	var self = this, colNames, dataObj = graphDef.data;
	var isJSON = AR.Utility.checkIfJSON(dataObj);
	if (isJSON) {
		colNames = AR.Utility.getColNames(dataObj);
		self._dataTable = new AR.Utility.Table(colNames, dataObj, true);
	}
	self._maxValue = AR.Utility.findMaxVal(self._dataTable,graphDef.measureAxis)
	self._outerPanel = new pv.Panel().left(0).right(0).bottom(0).top(0);
	self._panel = self._outerPanel.add(pv.Panel);
	self._panel.data(dataObj);
...
/**
 * Sets a graph's caption. Caption is one of the properties of any graph
 * 
 * @param {string}
 *            Sets the caption for the given graph as the supplied parameter
 *            value
 */
...
/**
 * Sets a graph's width. Width is one of the properties of a graph
 * 
 * @param {string}
 *            [width] Sets the width for the given graph as the supplied
 *            parameter value
 */
 ...
 * 
 * @param {html
 *            node} [div]The node in which the graph has to be rendered. If
 *            undefined grpah is rendered whereever the function is called.
 */
...
AR.Graph.prototype.setVerGridShow = function(status) {
	var self = this, maxValue = self._maxValue, scaleType = AR.Utility.scale.linear;
	if (status === true) {
		self._x = pv.Scale[scaleType](0, maxValue).range(0, self._dimension.width - 40);
		if (!self._verGrid) {
			self._verGrid = self._panel.add(pv.Rule);
			self._verGrid.data(self._x.ticks().splice(1, self._x.ticks().length)).left(self._x);
		} else {
...
AR.Graph.prototype.setHorGridShow = function(status) {
	var self = this, maxValue = self._maxValue, scaleType = AR.Utility.scale.linear;
	if (status === true) {
