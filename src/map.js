...
AR.Canvas.prototype.redraw = function(force) {
	...
	var x = { min: pv.min(pixels, getX) - r, max: pv.max(pixels, getX) + r };
	var y = { min: pv.min(pixels, getY) - r, max: pv.max(pixels, getY) + r };
	...
	if(this.graphDef.showLabels){
		var labels = this._dot.anchor("center").add(pv.Label)
	    .textStyle("white")
	    .text(function(x, d) d.category);
