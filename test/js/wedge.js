...
AR.Donut = function(parentDimension, panel, graphDef){
	...
	var setAnchor = function(datavals){
		var wedger = wedge.anchor("center").add(pv.Label).visible(function(d) {return d;});
		wedger.textAngle(0).text(function(d) { return (d.toFixed(2))});
	};
...
AR.WedgeGraph = function(graphDef){
	var self = this;
	AR.Graph.apply(self, [graphDef]);
	var wedges;
	if(graphDef.wedgeType == "pie"){
		var wedges = new AR.Wedge(self._dimension, self._panel, graphDef);
	}else{
		wedges = new AR.Donut(self._dimension, self._panel, graphDef);
	}
...
AR.WedgeGraph.prototype = AR.extend(AR.Graph);
